...
public class TutorialAction extends BaseAction implements Preparable {
    ...
    public void prepare() {
        ...
        if (getRequest().getMethod().equalsIgnoreCase("post") && (!"".equals(getRequest().getParameter("tutorial.id")))) {
            tutorial = tutorialManager.getTutorial(new Long(getRequest().getParameter("tutorial.id")));
        }
    }
    ...
    public String save() throws Exception {
        ...
        boolean isNew = ("".equals(getRequest().getParameter("tutorial.version")));
        ...
        if (getRequest().isUserInRole(Constants.ADMIN_ROLE) || getRequest().isUserInRole(Constants.TUTOR_ROLE)) {
            ...
            if (!"list".equals(from)) {
                ...
                if (isNew) {
                    ...
                    try {
                        sendUserMessage(userManager.getUserByUsername(request.getRemoteUser()), getText("newtutorial.email.message", args), RequestUtil.getAppURL(getRequest()));
